...
export const MOCK_MATH_PROBLEMS = [
  {
    ...
    analysis: {
      ...
      solutionMethod: [
        `**求集合 A**: 解不等式 $x^2 - 2x - 3 < 0$, 即 $(x-3)(x+1) < 0$, 解得 $-1 < x < 3$。`,
        `**求集合 B**: 由 $\\ln(2-x)$ 有意义，得 $2-x > 0$, 即 $x < 2$。`,
        `**求交集**: 取公共部分，得 $-1 < x < 2$。`,
        `故 $A \\cap B = (-1, 2)$。`
      ]
    }
  },
  {
    ...
    analysis: {
      ...
      solutionMethod: [
        `观察式子 $\\frac{\\sin \\alpha - 3\\cos \\alpha}{\\sin \\alpha + \\cos \\alpha}$，分子分母同时除以 $\\cos \\alpha$。`,
        `原式转化为 $\\frac{\\tan \\alpha - 3}{\\tan \\alpha + 1}$。`,
        `代入 $\\tan \\alpha = 2$，得 $\\frac{2 - 3}{2 + 1} = -\\frac{1}{3}$。`
      ]
    }
  },
  {
    ...
    analysis: {
      ...
      solutionMethod: [
        `**求周期**: 根据图像(假设)观察 $T/4$ 或 $T/2$，计算 $\omega$。`,
        `**求初相**: 代入最值点坐标求 $\phi$。`,
        `写出解析式后代入 $x = \frac{\pi}{6}$ 计算。`
      ]
    }
  }
];
